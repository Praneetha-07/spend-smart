...
export default function Dashboard() {
  ...
  // expenses: /expenses?userId=USER_ID
  ...
  useEffect(() => {
    ...
    fetchIncome();
    fetchExpenses();
  }, [currentUser, fetchIncome, fetchExpenses, navigate]);

  // ---------- MESSAGE BOX ----------
  const showMessageBox = useCallback(
    (text, confirm = false, callback = null) => {
      setMessage(text);
  ...
  // ---------- ADD EXPENSE (CREATE: POST) ----------
  const addExpense = async (e) => {
    e.preventDefault();
    const title = e.target.title.value.trim();
    const amountVal = parseFloat(e.target.amount.value);
    const category = e.target.category.value;

    if (!title || isNaN(amountVal) || amountVal <= 0) {
      return showMessageBox(
        "Enter valid expense details: a title and an amount greater than 0!"
      );
    }
    ...
    try {
      ...
      showMessageBox("Failed to add expense. Please try again.");
      return;
    }
  ...
  const deleteExpense = useCallback(
    async (id) => {
      try {
        ...
        showMessageBox("Failed to delete expense. Please try again.");
      }
    },
    [showMessageBox]
  );

  // ---------- SET INCOME (CREATE / UPDATE) ----------
  const setIncomeHandler = async (e) => {
    e.preventDefault();
    ...
    try {
      ...
      showMessageBox("Failed to set income. Please try again.");
      return;
    }
  ...
  // ---------- CLEAR ALL DATA (income + expenses) ----------
  const clearAllData = useCallback(async () => {
    try {
      ...
      showMessageBox("Failed to clear data. Please try again.");
    }
  }, [currentUser, incomeId, showMessageBox]);

  const requestClearData = () => {
    showMessageBox(
      "Are you sure you want to clear ALL income and expense data? This action cannot be undone.",
      true,
      clearAllData
    );
  };
  ...
  const getFilteredAndSortedExpenses = useCallback(() => {
    ...
    // dateDesc – sort by most recent date
    else
      sorted.sort(
  ...
  const updateCharts = useCallback(() => {
    ...
    const backgroundColors = Object.keys(totals).map(
    ...
    const balanceColor =
      balance >= 0 ? categoryColors.Food : categoryColors.Shopping;
  ...
  const renderExpenseList = () => {
    ...
    return sortedExpenses.map((exp) => {
      ...
      return (
        <li key={exp.id}>
          <span>
            <strong>{formatCurrency(exp.amount)}</strong> - {exp.title} (
            {exp.category})
  ...
  return (
    <>
      ...
      <div className="dashboard-container">
        ...
        >
          <div className="left-panel input-controls">
            <div className="card summary">
              ...
              <span>
                Income:{" "}
                <strong id="incomeAmount">{formatCurrency(income)}</strong>
              </span>
              <br />
              <span>
                Expenses:{" "}
            ...
            <button className="clear-btn" onClick={requestClearData}>
              Reset All Data
            </button>
          </div>
          ...
          <div className="left-panel list-view">
            ...
            <ul id="expenseList">{renderExpenseList()}</ul>
          </div>
        </div>
        ...
        <div
          id="reports"
          className={`tab-content ${activeTab === "reports" ? "active" : ""}`}
        >
